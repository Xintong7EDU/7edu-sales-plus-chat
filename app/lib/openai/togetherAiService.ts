...
// Together AI API key from environment
const apiKey = process.env.TOGETHER_AI_API_KEY || 'dummy-key-for-development';
const togetherApiBaseUrl = 'https://api.together.xyz/v1';

/**
 * Configuration options for Together AI chat completions
 */
 ...
 * @param advancedMode Whether to use the detailed system prompt
...
/**
 * Generates a non-streaming chat completion using Together AI
 * 
 * @param messages Array of messages for the conversation history
 * @param userProfile User profile with onboarding information
 * @param options Configuration options for the API call
 * @returns The AI-generated completion text
 */
export async function generateTogetherAiChatCompletion(
  messages: ChatCompletionMessageParam[],
  userProfile: UserProfile,
  options: ChatOptions = {}
): Promise<string> {
  console.log('Generating Together AI chat completion...');
  console.time('togetherAiCompletion');
  ...
  try {
    ...
    // Prepare system message with context about the student
    const systemMessage = createSystemMessage(userProfile, advancedMode);
    ...
    const response = await fetch(`${togetherApiBaseUrl}/chat/completions`, {
      method: 'POST',
      headers: {
    ...
    console.timeEnd('togetherAiCompletion');
...
/**
 * Generates a streaming chat completion using Together AI
 * 
 * @param messages Array of messages for the conversation history
 * @param userProfile User profile with onboarding information
 * @param options Configuration options for the API call
 * @returns A ReadableStream for consuming the streamed response
 */
export async function streamTogetherAiChatCompletion(
  messages: ChatCompletionMessageParam[],
  userProfile: UserProfile,
  options: ChatOptions = {}
) {
  console.log('Streaming Together AI chat completion...');
      ...
      async start(controller) {
        try {
          ...
          if (!response.ok) {
            ...
            controller.error(new Error(`Together AI API error: ${response.status} ${response.statusText}`));
            return;
          }
          ...
          if (!reader) {
            controller.error(new Error('Failed to get stream reader from Together AI response'));
            return;
          }
          ...
          console.error('Error streaming Together AI completion:', error);
          controller.error(error);
        }
      }
    });
  } catch (error) {
    console.error('Error setting up Together AI stream:', error);
    throw error;
  }
} 